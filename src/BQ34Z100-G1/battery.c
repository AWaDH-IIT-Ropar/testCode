...
static int  read_control(uint16_t control_subcommand)
{
    uint8_t n_bytes = 2;
    uint8_t reg_data[2];
    //Divide 16 bit address into two 8 bits
    reg_data[0] = control_subcommand & 0x00FF; //lsb
    reg_data[1] = control_subcommand >> 8; //msb
    //To write address of particular register for reading value
    ...
    //This will read the value of the register
...
// for checking the observed data whether it is right or wrong
